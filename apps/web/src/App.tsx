...
export default function App() {
  ...
  const [statusFilter, setStatusFilter] = useState<Question["status"] | null>(
    null,
  );

  const loadQuestions = async (status: Question["status"] | null = null) => {
    setLoading(true);
    setError("");
    try {
  ...
  useEffect(() => {
    socket.on("newQuestion", (q: Question) =>
      setQuestions((prev) => [q, ...prev]),
    );
    socket.on("updateQuestion", (q: Question) =>
      setQuestions((prev) => prev.map((p) => (p._id === q._id ? q : p))),
    );
    socket.on("deleteQuestion", (id: string) =>
  ...
  const addQuestion = async (payload: QuestionCreatePayload) => {
    try {
  ...
  const patchQuestion = async (id: string, body: QuestionUpdatePayload) => {
    try {
  ...
  const doClear = async (onlyAnswered: boolean) => {
